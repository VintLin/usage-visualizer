...
def generate_html_report(
    ...
    for record in all_data:
        ...
        if prev_start_date <= date <= (requested_start - timedelta(days=1)).strftime("%Y-%m-%d"):
    ...
    def get_trend_svg(dates, key, width=580, height=100):
        ...
        for idx, m in enumerate(top_trend_models):
            ...
            svg += f'<path d="M{"L".join(m_pts)}" fill="none" stroke="{colors[idx+1]}" stroke-width="1.5" stroke-dasharray="4,3" opacity="0.8" />'
        return svg + '</svg>'
            ...
            <div style="background:#1a1a1a;border-radius:24px;padding:32px;border:1px solid #2a2a2a;flex:1">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px">
                    ...
                    <div style="display:flex;gap:12px;font-size:10px">
                        <div style="display:flex;align-items:center;gap:5px"><span style="width:10px;height:2px;background:{colors[0]}"></span>Total</div>
